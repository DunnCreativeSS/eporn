...
app.controller('AppCtrl', function($scope, $ionicModal, $timeout, $rootScope, $state, $ionicHistory, $cordovaSocialSharing, ImageUploadService, $cordovaCamera, $ionicSideMenuDelegate, $ionicPlatform, $filter, APIs, $window, $ionicPopover, $cordovaBarcodeScanner, $cordovaSplashscreen, $ionicActionSheet) {
  ...
  $ionicPlatform.ready(function(){
    if (window.cordova) {
      ...
      $rootScope.$storage.appversion = '1.4.6';
    }
  });
  ...
  $scope.testfunction2 = function() {
    ...
    if ($rootScope.user) {
      ...
      var permlink = "tests";
      var json = {};
      angular.merge(json, {tags: ['porn'], app: 'porn/'+$rootScope.$storage.appversion, format: 'markdown+html', community: 'esteem' });
      ...
      operations_array = [
        ...
        ['comment_options', {
          ...
          extensions: [[0, { "beneficiaries": [{ "account":"hodlorbust", "weight":5000 }] }]]
        }]
        ];
...
app.controller('PostsCtrl', function($scope, $rootScope, $state, $ionicPopup, $ionicPopover, $interval, $ionicScrollDelegate, $ionicModal, $filter, $stateParams, $ionicSlideBoxDelegate, $ionicActionSheet, $ionicPlatform, $cordovaCamera, ImageUploadService, $filter, $ionicHistory, APIs, $translate, $compile) {
  ...
  $scope.scheduleStory = function() {
    ...
    if ($rootScope.user) {
      ...
      angular.merge($scope.spost.json, {tags: ['porn'], app: 'porn/'+$rootScope.$storage.appversion, format: 'markdown+html', community: 'esteem' });
      ...
      if ($scope.spost.operation_type !== 'default') {
        operations_array = [
          ...
          ['comment_options', {
            ...
            extensions: $rootScope.$storage.chain == 'golos'?[]:[[0, { "beneficiaries": [{ "account":"hodlorbust", "weight":5000 }] }]]
          }]
          ];
  ...
  $scope.savePost = function() {
    ...
    //adddraft
    var ttt = $scope.spost.tags
    ttt.push('porn')
    var dr = {title:$scope.spost.title, body: $scope.spost.body, tags: ttt, post_type: $scope.spost.operation_type};
        ...
        for (var i = 0; i < lenn; i++) {
          ...
          if (view === 'card') {
            if (newValue[i].json_metadata){
              if (typeof newValue[i].json_metadata === 'string' || newValue[i].json_metadata instanceof String) {
                // it's a string
  ...
  $scope.fetchContent = function(author, permlink) {
    window.steem.api.getContentAsync(author, permlink, function(err, result) {
      ...
      if (typeof result.json_metadata === 'string' || result.json_metadata instanceof String) {
        // it's a string
        ...
        if (response) {
          ...
          //$scope.$evalAsync(function($scope){
            ...
            for (var i = 0; i < response.length; i++) {
              ...
              if (response[i]){
                let r = rp("https://scot-api.steem-engine.com/@" + response[i].author + "/" + permlink)
                response[i].total = JSON.parse(r).total_payout_value
                response[i].pending - JSON.parse(r).pending_payout_value
              }
...
app.controller('PostCtrl', function($scope, $stateParams, $rootScope, $interval, $ionicScrollDelegate, $ionicModal, $filter, $ionicActionSheet, $cordovaCamera, $ionicPopup, ImageUploadService, $ionicPlatform, $ionicSlideBoxDelegate, $ionicPopover, $filter, $state, APIs, $ionicHistory, $ionicPosition, $cordovaFileTransfer, $ionicLoading, $translate) {
  ...
  $scope.submitStory = function() {
    ...
    if ($rootScope.user) {
      ...
      //console.log(jjson);
      //$scope.spost.tags = $filter('lowercase')($scope.spost.tags);
      var ttt = $scope.spost.tags.split(" ")
      ttt.push('porn')
      var json = angular.merge(jjson, {tags: ttt, app: 'porn/'+$rootScope.$storage.appversion, format: 'markdown+html', community: 'esteem' });
  ...
  $scope.reply = function (xx) {
    ...
    if ($rootScope.user) {
      if ($scope.editreply) {
        ...
        var json;
        if (Array.isArray(ts)) {
          json = {tags: ["porn"] , app: 'porn/'+$rootScope.$storage.appversion, format: 'markdown+html', community: 'esteem' };
        } else {
          json = {tags: ["porn"] , app: 'porn/'+$rootScope.$storage.appversion, format: 'markdown+html', community: 'esteem' };
        }
        ...
        var t = new Date();
        var timeformat = t.getFullYear().toString()+(t.getMonth()+1).toString()+t.getDate().toString()+"t"+t.getHours().toString()+t.getMinutes().toString()+t.getSeconds().toString()+t.getMilliseconds().toString()+"z";
        var json = {tags: ["porn"] , app: 'porn/'+$rootScope.$storage.appversion, format: 'markdown+html', community: 'esteem' };
  ...
  $scope.fetchComments = function(author, permlink){
    ...
    window.steem.api.getStateAsync('tag/@'+author+'/'+permlink, function(err, dd) {
      //console.log(dd);
      var po = [];
      $rootScope.paccounts = {};
      if (dd) {
